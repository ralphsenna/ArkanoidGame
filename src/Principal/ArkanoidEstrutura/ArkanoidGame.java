...
public class ArkanoidGame extends Pane {
    ...
    int indice; //indice a ser sorteado para definir tipo/cor do bloco
    String texto; //texto a ser apresentado "Você ganhou", "Você perdeu" ou "[p] para despausar"
    Media soundGanhou, soundPerdeu, soundbate, fundo, sound;
    MediaPlayer mediaPlayer, mediaPlayerFundo, mediaPlayerInicio;
    File pastaBlocos = new File("src\\Principal\\img\\blocos");
    File imgBlocos[] = pastaBlocos.listFiles();
    public ArkanoidGame() {
        super();
        Image image;
        try {
            ...
            soundPerdeu = new Media(new File("src\\Principal\\audios\\audioPerdeu.mp3").toURI().toString());
            soundbate = new Media(new File("src\\Principal\\audios\\som_batebolinha.mp3").toURI().toString());
            fundo = new Media(new File("src\\Principal\\audios\\som_element_fundo.mp3").toURI().toString());
            sound = new Media(new File("src\\Principal\\audios\\efect_pulo.mp3").toURI().toString());
           
            //definindo o background
            image = new Image(new FileInputStream("src\\Principal\\img\\backgroundPreto.jpg"));
    ...
    private void iniciar() {
        ...
        exibeBlocos(blocos); //função que sorteia os blocos a serem inseridos e os adiciona na tela
        adicionaBolinha();        
        jogar();  
    }
    ...
    private boolean finalizar() {
        ...
        {
            texto = "Você Perdeu =(";
            mediaPlayer = new MediaPlayer(soundPerdeu); 
            mediaPlayer.play();//soundPerdeu
        }
        else
        if(vidas>0 && blocos.size()==0){ //verifica
            texto = "Você ganhou =)";
    ...
    private void controlaBolinha() {
        for(Bolinha bol:b) {
            ...
            if(bol.verColisao(raquete)){ //Verifica se houve colisão da bolinha com a raquete e muda sua direção
                bol.mudarDirecaoY();
                mediaPlayer = new MediaPlayer(soundbate); 
                mediaPlayer.play();
            }
        } 
    }

    private void exibeBlocos(List<AbstractBloco> blocos) {
        Image image;
        ImagePattern image_pattern;
        int col;
        try {
            //primeiro bloco para referência 
            image = new Image(new FileInputStream("src\\Principal\\img\\blocos\\"+imgBlocos[2].getName()));
            ...
            for(int linha = 0; linha < 5; linha++){
                ...
                while(blocos.get(blocos.size()-1).getFigura().getX() + 2*(blocos.get(0).getFigura().getWidth() + m) <= this.getWidth()){
                    ...
                    if(col == 3 && NovoNivel.getNivel() == 2){
                        ...
                        indice = (int) (Math.random()*imgBlocos.length);
                        defineBloco();
                        image = new Image(new FileInputStream("src\\Principal\\img\\blocos\\"+imgBlocos[indice].getName()));
                    }
                    image_pattern = new ImagePattern(image);
                    blocos.get(blocos.size()-1).getFigura().setFill(image_pattern); 
                    getChildren().add(blocos.get(blocos.size()-1).getFigura());
    ...
    private void defineBloco(){
        if(imgBlocos[indice].getName().endsWith("Amarelo.png"))
            bAmarelo();    
        else if(imgBlocos[indice].getName().endsWith("Verde.png"))
            bVerde();
        else if(imgBlocos[indice].getName().endsWith("Azul.png"))
           bAzul();
        else if(imgBlocos[indice].getName().endsWith("Branco.png"))
            bBranco();
        else
            bVermelho();
    }
